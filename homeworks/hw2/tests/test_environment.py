...
@pytest.mark.parametrize("heap_amount", heaps_amounts, ids=heaps_amounts_ids)
...
def test_get_state_return_value():
    ...
    assert all(STONE_AMOUNT_MIN <= elem <= STONE_AMOUNT_MAX for elem in state)


@pytest.mark.parametrize("state_change", state_changes, ids=state_changes_ids)
